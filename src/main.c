...
void check(int argc, char **argv) {
  for (int i = 1; i < argc; i++) {
    if (argv[i][0] != '-') {
      ...
      char s = argv[i][1];
      if (s == 'r')
        glo_flag.reloc_table = 1;
      else if (s == 's')
        glo_flag.glo_sym_table = 1;
      else if (s == 'o') {
        glo_flag.need_obj = 1;
        i++;
        glo_flag.dst = argv[i];
        ...
        i++;
        glo_flag.jit_pg_no = atoi(argv[i]);
        if(glo_flag.jit_pg_no == 0){
...
int main(int argc, char **argv) {
  ...
  printf("JIT returned: %dbytes / %d % \n", entry->jit_cur,
         entry->jit_cur * 100 / entry->jit_compiled_len);
